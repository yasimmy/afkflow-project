...
class CookingWorker(BaseWorker):
    ...
    def _perform_click(self, coords: Tuple[int, int], left_click: bool = False) -> None:
        """Быстро выполняет клик по ячейке или инструменту."""
        try:
            import pyautogui
...
class CookingBotApp(BaseBotApp):
    ...
    def _on_cooking_status(self, status: str) -> None:
        """Keep worker progress from replacing the paused state in the overlay."""
        if self._cooking_state == "PAUSED":
            return
    ...
    def _pause_cooking(self) -> None:
        """Пауза готовки"""
        if self._cooking_state == "RUNNING" and self._worker and hasattr(self._worker, 'pause'):
            self._worker.pause()
            self._cooking_state = "PAUSED"
            self._update_status("На паузе")
    
    def _resume_cooking(self) -> None:
        """Продолжение готовки"""
        if self._cooking_state == "PAUSED" and self._worker and hasattr(self._worker, 'resume'):
            self._worker.resume()
            self._cooking_state = "RUNNING"
    ...
    def _start_cooking(self) -> None:
        """Запуск готовки"""
        if self._cooking_state in {"RUNNING", "PAUSED"} or self._running:
            return
        ...
        if self._worker:
            ...
            self._worker.status_updated.connect(self._on_cooking_status)
    ...
    def _on_overlay_f7(self) -> None:
        if self._cooking_state == "PAUSED" and self._worker:
            self._resume_cooking()
            return
        if self._cooking_state == "IDLE" and not self._running:
            self._start_cooking()

    def _on_overlay_f8(self) -> None:
        if self._cooking_state == "RUNNING":
            self._pause_cooking()
