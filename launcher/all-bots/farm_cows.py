...
class FarmCowsWorker(BaseWorker):
    ...
    def run(self) -> None:
        ...
        while self.is_running:
            ...
            # Во время мини-игры E не нажимаем: она используется только между играми.
            if not white_present and time.monotonic() >= next_e_press:
                self.status_updated.emit("Нажимаю E")
                press_key('e')
                next_e_press = time.monotonic() + 1.5
            ...
            while white_present and self.is_running:
                ...
                if self._check_danger_color():
                    if not self._force_pause:
                        ...
                        self.status_updated.emit("Опасность")
                        self.log_message.emit("ВНИМАНИЕ: Обнаружен опасный цвет")
                    
                    pause_time = random.uniform(5, 15)
                    self.status_updated.emit("Пауза")
                    self.log_message.emit(f"Пауза из-за опасного цвета: {pause_time:.1f}с")
                ...
                if a_detected and d_detected:
                    ...
                    self.status_updated.emit("Нажимаю A")
                    if not self.wait_if_paused():
                        break
                    ...
                    self.status_updated.emit("Нажимаю D")
                    if not self.wait_if_paused():
                        break
