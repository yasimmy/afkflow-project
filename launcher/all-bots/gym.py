...
from PyQt5.QtGui import QPainter, QColor, QPen, QPainterPath, QKeySequence
...
from components.styles import (
    ...
    FRAME_STYLES, INPUT_STYLES, GROUPBOX_STYLES,
)
...
from components.bot_overlay import (
    ...
    OVERLAY_STATUS_FOOD_CHECK,
    OVERLAY_STATUS_FEEDING,
    OVERLAY_STATUS_READY,
    OVERLAY_STATUS_SOLVE,
    OVERLAY_STATUS_WAIT,
    OverlayIcon,
...
class GymWorkerThread(QThread):
    ...
    def _open_menu_and_check_hunger(self) -> bool:
        ...
        """
        try:
            self.update_status.emit(OVERLAY_STATUS_FOOD_CHECK)
    ...
    def _feed_character(self):
        ...
        try:
            ...
            self.update_status.emit(OVERLAY_STATUS_FEEDING)
    ...
    def _perform_approach(self, cycle_counter: int) -> bool:
        ...
        self.update_status.emit(OVERLAY_STATUS_WAIT)
        self.log_message.emit(OVERLAY_STATUS_WAIT)
        ...
        while self.running and not success_detected and not fail_detected:
            ...
            if self.space_pressed_flag:
                self.update_status.emit(OVERLAY_STATUS_SOLVE)
                self.log_message.emit(OVERLAY_STATUS_SOLVE)
                self.space_pressed_flag = False
    ...
    def run(self):
        try:
            self.detector_thread = threading.Thread(
                ...
                args=(self.on_space_pressed, self.miss_chance, self.detector_stop_event),
                daemon=True
            )
...
class GymApp(QWidget):
    ...
    def initUI(self):
        ...
        self.icon = OverlayIcon("dumbbell", self)
        overlay.addWidget(self.icon, 0, Qt.AlignVCenter)
        ...
        top_row.addWidget(self.counter_label, 0, Qt.AlignRight | Qt.AlignVCenter)
        text_col.addLayout(top_row)

        self.status_label = QLabel(OVERLAY_STATUS_IDLE)
        self.status_label.setStyleSheet(self._status_stylesheet(OVERLAY_STATUS_IDLE))
        ...
        self.setFixedSize(268, 78)
        self._move_to_top_right()

        # Настройки остаются в коде, но не показываются — позже включатся в лаунчере
        ...
        self.settings_status_label = QLabel("Состояние: не активно")
        self.settings_status_label.setStyleSheet(LABEL_STYLES["primary"])
        self.settings_status_label.setAlignment(Qt.AlignCenter)
        main_layout.addWidget(self.settings_status_label)
    ...
    def _status_stylesheet(self, text: str) -> str:
        return overlay_status_style(text)

    def set_overlay_status(self, text: str):
        self.status_label.setText(text)
        self.status_label.setStyleSheet(self._status_stylesheet(text))
    ...
    def _setup_shortcuts(self):
        ...
        self._f7.setContext(Qt.ApplicationShortcut)
        self._f7.activated.connect(self._on_f7)
        self._f8 = QShortcut(QKeySequence("F8"), self)
        self._f8.setContext(Qt.ApplicationShortcut)
        self._f8.activated.connect(self._on_f8)
    ...
    def _setup_global_hooks(self):
        if KEYBOARD_LIB == "pynput":
            def on_press(key):
                try:
                    ...
                    if key == pynput_keyboard.Key.f7 or vk == 0x76 or name == "f7":
                        QTimer.singleShot(0, self._on_f7)
                    elif key == pynput_keyboard.Key.f8 or vk == 0x77 or name == "f8":
                        QTimer.singleShot(0, self._on_f8)
                except Exception:
                    pass
            ...
            self.add_log("Глобальные хоткеи F7/F8 включены")
        elif KEYBOARD_LIB == "keyboard":
            import keyboard as keyboard_lib
            keyboard_lib.add_hotkey("f7", lambda: QTimer.singleShot(0, self._on_f7))
            keyboard_lib.add_hotkey("f8", lambda: QTimer.singleShot(0, self._on_f8))
            self.add_log("Глобальные хоткеи F7/F8 включены")
        else:
            self.add_log("F7/F8 вне фокуса недоступны: установите pynput")

    def _on_f7(self):
        if self.running and self.worker_thread and self.worker_thread.paused:
    ...
    def _on_f8(self):
        if self.running and self.worker_thread and not self.worker_thread.paused:
            self.pause_training()
        ...
        try:
            ...
            if not espander:
                try:
                    ...
                    self.settings_status_label.setText("Ошибка: некорректное время между подходами")
                    self.set_overlay_status("Ошибка настроек")
                    return
                
            self.running = True
            self.toggle_button.setText("Остановить тренировку")
            self.toggle_button.setStyleSheet(BUTTON_STYLES["danger"])
            self.settings_status_label.setText("Состояние: Тренировка активна")
            self.set_overlay_status(OVERLAY_STATUS_READY)
            
            self.set_controls_enabled(False)
            
            self.reset_counter()
            self.set_total_cycles(cycles)
            ...
            self.settings_status_label.setText(error_message)
            self.set_overlay_status("Ошибка запуска")
    ...
    def pause_training(self):
        if self.running and self.worker_thread:
            ...
            self.settings_status_label.setText("Состояние: Пауза")
            self.set_overlay_status(OVERLAY_STATUS_PAUSE)

    def resume_training(self):
        if self.running and self.worker_thread:
            self.worker_thread.resume()
            self.settings_status_label.setText("Состояние: Тренировка активна")
            self.set_overlay_status(OVERLAY_STATUS_WAIT)

    def stop_training_async(self):
        self.stop_requested = True
        self.toggle_button.setEnabled(False)
        self.toggle_button.setText("Останавливается...")
        self.settings_status_label.setText("Состояние: Остановка...")
    ...
    def finalize_stop(self):
        ...
        self.settings_status_label.setText("Состояние: Не активно")
        self.set_overlay_status(OVERLAY_STATUS_IDLE)
    ...
    def increment_counter(self):
        ...
        self.add_log(f"Счётчик: {self.counter}")
    ...
    def update_status_label(self, message):
        self.settings_status_label.setText(f"Состояние: {message}")
        if message in (
            OVERLAY_STATUS_READY,
            OVERLAY_STATUS_WAIT,
            OVERLAY_STATUS_SOLVE,
            ...
            self.set_overlay_status(message)
